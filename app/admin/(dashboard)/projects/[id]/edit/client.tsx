...
export function EditProjectClient({ project }: EditProjectClientProps) {
  ...
  const removeGalleryImage = (index: number) => {
    // If it's a newly added file (exists in galleryFiles), remove it from there
    // If it's an existing URL, we just remove it from previews and it will be updated in DB
    const previewToRemove = galleryPreviews[index]
    
    setGalleryPreviews(prev => prev.filter((_, i) => i !== index))
    
    // Find if this preview is associated with a new file
    if (previewToRemove.startsWith('blob:')) {
      // Find the index in the galleryFiles array
      // This is an approximation: we assume the order of blob URLs matches the order of galleryFiles
      const blobPreviews = galleryPreviews.filter(p => p.startsWith('blob:'))
      const fileIndex = blobPreviews.indexOf(previewToRemove)
      if (fileIndex !== -1) {
  ...
  const onSubmit = async (data: ProjectFormData) => {
    ...
    // Keep existing URLs and upload new files
    let galleryUrls: string[] = galleryPreviews.filter(p => !p.startsWith('blob:'))
    ...
    const techStack = data.tech_stack.split(',').map((t) => t.trim()).filter(Boolean)
    ...
    <div className="p-8 max-w-3xl">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-text-secondary text-sm font-mono">{project.slug}</p>
        </div>
      </div>
      ...
      >
        <div>
          <label htmlFor="edit-title" className={labelClass}>Project Title *</label>
          <input id="edit-title" type="text" className={inputClass}
            {...register('title', { required: 'Title is required' })} onChange={onTitleChange} />
          {errors.title && <p className={errorClass}>{errors.title.message}</p>}
        </div>

        <div>
          <label htmlFor="edit-slug" className={labelClass}>Slug *</label>
          <input id="edit-slug" type="text" className={inputClass}
            {...register('slug', { required: true })} />
        </div>

        <div>
          <label htmlFor="edit-role" className={labelClass}>Role</label>
          <input id="edit-role" type="text" className={inputClass} {...register('role')} />
        </div>

        <div>
          <label htmlFor="edit-tech" className={labelClass}>Tech Stack</label>
          <input id="edit-tech" type="text" placeholder="Next.js, Tailwind, Supabase" className={inputClass} {...register('tech_stack')} />
        </div>

        <div>
          <label htmlFor="edit-link" className={labelClass}>Live URL</label>
          <input id="edit-link" type="url" className={inputClass} {...register('live_link')} />
        </div>
        ...
        <div>
          ...
          <div className="relative mb-6">
            ...
            <label htmlFor="edit-image"
              className="flex flex-col items-center justify-center w-full h-36 border-2 border-dashed border-border-color rounded-xl cursor-pointer hover:border-teal/40 transition-colors bg-black/5 dark:bg-white/5 overflow-hidden">
              {imagePreview ? (
        ...
        <div className="flex items-center gap-3">
          <input id="edit-featured" type="checkbox" className="w-4 h-4 rounded accent-teal" {...register('featured')} />
          <label htmlFor="edit-featured" className="text-sm font-medium text-text-primary">Feature on home page</label>
        </div>

        <div>
          <label htmlFor="edit-content" className={labelClass}>Project Description</label>
          <textarea id="edit-content" rows={6} placeholder="Describe the project..." className={`${inputClass} resize-y leading-relaxed`} {...register('content')} />
